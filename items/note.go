...
func (n Notes) Validate(sess *session.Session) error {
...
func (noteContent *NoteContent) GetUpdateTime() (time.Time, error) {
	...
	return time.Parse(common.TimeLayout, noteContent.AppData.OrgStandardNotesSN.ClientUpdatedAt)
}

func (noteContent *NoteContent) SetUpdateTime(uTime time.Time) {
	noteContent.AppData.OrgStandardNotesSN.ClientUpdatedAt = uTime.Format(common.TimeLayout)
}
